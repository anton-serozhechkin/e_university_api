...
class TestReadUserRequestList:
    async def test_read_user_request_list_200(
        ...
        for user_request in [user_request_1, user_request_2]:
            ...
            assert (
                created_instance.get("user_request_id") == user_request.user_request_id
            )
            assert (
                created_instance.get("service_name")
                == user_request.service.service_name
            )
            assert created_instance.get("created_at") == (
                user_request.created_at.strftime("%Y-%m-%d")
                + "T"
                + user_request.created_at.strftime("%H:%M:%S")
                + "+00:00"
            )
...
class TestReadHostelAccommodation:
    async def test_read_hostel_accommodation_200(
        ...
        assert (
            data.get("user_request_review_id")
        ...
        assert (
            data.get("bed_place_name") == user_request_review.bed_place.bed_place_name
        )
        assert data.get("month_price") == float(user_request_review.hostel.month_price)
        assert (
            data.get("start_accommodation_date")
            == user_request_review.start_accommodation_date
        )
        assert (
            data.get("end_accommodation_date")
            == user_request_review.end_accommodation_date
        )
        assert data.get("total_sum") == float(user_request_review.total_sum)
        assert data.get("iban") == requisites.iban
        assert data.get("university_name") == university.university_name
        assert data.get("organisation_code") == requisites.organisation_code
        assert data.get("payment_recognition") == requisites.payment_recognition
        ...
        assert data.get("documents") == service_document.documents
...
class TestReadUserDocumentList:
    async def test_read_user_document_list_200(
        ...
        response = await async_client.get(
            url=app_fixture.url_path_for(
                name="read_user_documents_list", university_id=university.university_id
            ),
            headers={"Authorization": f"Bearer {token}"},
        )
        assert_jsend_response(
        ...
        for user_document in user_documents:
            ...
            assert (
                created_instance.get("user_document_id")
            ...
            assert created_instance.get("created_at") == (
                ...
                + "T"
                + user_document.created_at.strftime("%H:%M:%S.%f")
                + "+00:00"
            )
            assert created_instance.get("updated_at") == (
                user_document.updated_at.strftime("%Y-%m-%d")
                + "T"
                + user_document.updated_at.strftime("%H:%M:%S.%f")
                + "+00:00"
            )
...
class TestReadUserRequestDetails:
    async def test_read_user_request_details_200_full(
        ...
        assert data.get("hostel_name") == {"name": hostel.name, "number": hostel.number}
    ...
    async def test_read_user_request_details_200_empty(
        ...
        assert data.get("status_id") == request_status.status_id
        assert data.get("comment") == user_request_out.get("comment")
        assert data.get("hostel_name") == {"name": None, "number": None}
...
class TestCreateStudentsListFromFile:
    async def test_create_students_list_from_file_200(
        ...
        file_path = Path(
            settings.BASE_DIR / "tests" / "samples" / "Form.xlsx"
        ).as_posix()
        with io.open(file_path, "rb", buffering=0) as f:
            data = f.read()
        upload_file = UploadFile(filename="Form.xlsx", file=io.BytesIO(data))
        ...
        for i in range(row, len(worksheet.col(1))):
            ...
            if not faculty_list:
                faculty: Faculty = FacultyFactory(
                    university_id=university.university_id, shortname=cell(i, col + 7)
                )
            faculty_id = (
            ...
            speciality_id = (
                speciality_list[0].speciality_id
                if speciality_list
