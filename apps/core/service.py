...
class AsyncCRUDBase:
    ...
    #     return data
# 
    # async def create_many(self, *, session: AsyncSession, objs: list[CreateSchemaType]):
    #     insert_statement = (
    #         insert(self.model)
    ...
    #     result: ChunkedIteratorResult = await session.execute(statement=statement)
    #     await session.commit()
    #     data: list[ModelType] = result.scalars().all()
    #     return data
# 
    # async def read(self, *, session: AsyncSession, id: str) -> ModelType :
    #     statement = select(self.model).where(self.model.id == id)
    #     result: ChunkedIteratorResult = await session.execute(statement=statement)
    #     data: ModelType = result.scalar_one_or_none()
    #     return data

    async def update(self, *, session: AsyncSession, id: str, obj: UpdateSchemaType) -> ModelType:
    ...
    async def delete(self, *, session: AsyncSession, id: str) -> CursorResult:
